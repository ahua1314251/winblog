...
package org.winker.winweb.winblog.config;
 ...
 * using Spring's {} for web MVC. It exists purely to configure
...
public class VelocityConfigurerCustom extends org.springframework.ui.velocity.VelocityEngineFactory
	 ...
	 * loader in order to load the spring Velocity macros from the class path.x
		...
		if (isPreferFileSystemAccess()) {
			...
			try {
				...
				for (int i = 0; i < paths.length; i++) {
					String path = paths[i];
					ResourceLoader resourceLoader =  new PathMatchingResourcePatternResolver(this.getClass().getClassLoader());
